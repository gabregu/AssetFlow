...
export const resolveTicketServiceDetails = (ticket, globalAssets) => {
    ...
    // Get hardware list from either assignedEquipments or associatedAssets
    const hardwareList = ticket.assignedEquipments || ticket.associatedAssets || [];
...
export const calculateTicketFinancials = (ticket, rates, globalAssets, users = [], logisticsTasks = []) => {
    ...
    if ((method === 'N/A' || !deliveryPerson) && logisticsTasks.length > 0) {
        let relatedTasks = logisticsTasks.filter(task => task.ticketId === ticket.id);
        
        // Also try matching by associated cases
        if (relatedTasks.length === 0 && ticket.associatedCases) {
             const caseNumbers = ticket.associatedCases.map(c => c.caseNumber);
             relatedTasks = logisticsTasks.filter(task => 
                 caseNumbers.includes(task.case_number) || caseNumbers.includes(String(task.ticketId))
             );
        }

        // Also try matching case_number with ticket.id
        if (relatedTasks.length === 0) {
             relatedTasks = logisticsTasks.filter(task => String(task.case_number) === String(ticket.id));
        }
