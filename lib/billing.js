...
export const calculateTaskFinancials = (task, rates, globalAssets = [], users = []) => {
    ...
    // ── DRIVER COST OVERRIDE ──────────────────────────────────────────────────
    // If the task has a manual override set from CaseLogisticsSection, use it
    // as the final logisticCost (replaces auto-calculated rate).
    const rawOverride = task.driver_cost_override ?? task.driverCostOverride;
    if (rawOverride !== null && rawOverride !== undefined && rawOverride !== '') {
        const parsed = parseFloat(rawOverride);
        if (!isNaN(parsed)) {
            logisticCost = parsed;
            extraDriverCost = 0; // override already is the full amount
        }
    }
