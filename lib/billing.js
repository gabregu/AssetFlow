...
export const calculateTicketFinancials = (ticket, rates, globalAssets, users = [], logisticsTasks = []) => {
    ...
    if (isWarranty) {
        ...
        const ticketDate = ticket.deliveryCompletedDate || ticket.closedDate || ticket.date;
        const exRate = getExchangeRateForDate(rates, ticketDate) || parseFloat(rates?.exchangeRate) || 1000;
        let rateInArs = 5000; // default 5000 ARS
        if (rates?.service_Other !== undefined && rates?.service_Other !== null && rates?.service_Other !== '') {
            rateInArs = parseFloat(rates.service_Other);
        } else if (rates?.otherService !== undefined && rates?.otherService !== null && rates?.otherService !== '') {
            rateInArs = parseFloat(rates.otherService) * exRate;
        }
        serviceRevenue = rateInArs / exRate;
    }
    ...
    if (relatedTasks.length > 0) {
        ...
        if (ticket.deliveryDetails?.customServiceRevenue !== undefined && ticket.deliveryDetails?.customServiceRevenue !== null && ticket.deliveryDetails?.customServiceRevenue !== '') {
            totalSRevenue = parseFloat(ticket.deliveryDetails.customServiceRevenue);
        }
        if (ticket.deliveryDetails?.customLogisticRevenue !== undefined && ticket.deliveryDetails?.customLogisticRevenue !== null && ticket.deliveryDetails?.customLogisticRevenue !== '') {
            totalLRevenue = parseFloat(ticket.deliveryDetails.customLogisticRevenue);
        }
    ...
    if (ticket.deliveryDetails?.customServiceRevenue !== undefined && ticket.deliveryDetails?.customServiceRevenue !== null && ticket.deliveryDetails?.customServiceRevenue !== '') {
        serviceRevenue = parseFloat(ticket.deliveryDetails.customServiceRevenue);
    }
    if (ticket.deliveryDetails?.customLogisticRevenue !== undefined && ticket.deliveryDetails?.customLogisticRevenue !== null && ticket.deliveryDetails?.customLogisticRevenue !== '') {
        logisticRevenue = parseFloat(ticket.deliveryDetails.customLogisticRevenue);
    }
...
export const calculateTaskFinancials = (task, rates, globalAssets = [], users = []) => {
    ...
    if (isLaptop) {
        ...
        const taskDate = task.date || task.created_at || new Date();
        const exRate = getExchangeRateForDate(rates, taskDate) || parseFloat(rates?.exchangeRate) || 1000;
        let rateInArs = 5000; // default 5000 ARS
        if (rates?.service_Other !== undefined && rates?.service_Other !== null && rates?.service_Other !== '') {
            rateInArs = parseFloat(rates.service_Other);
        } else if (rates?.otherService !== undefined && rates?.otherService !== null && rates?.otherService !== '') {
