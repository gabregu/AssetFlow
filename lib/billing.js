...
/**
 * Devuelve la cotización del dólar correspondiente al mes de una fecha dada.
 * Usa el historial de cotizaciones guardado en rates.exchangeRateHistory.
 * Si no hay historial para ese mes, busca el mes anterior más cercano.
 * Si no hay historial en absoluto, usa el valor global rates.exchangeRate.
 *
 * @param {object} rates - El objeto de tarifas del store
 * @param {string|Date} date - La fecha del ticket (ISO string o Date)
 * @returns {number} La cotización ARS del dólar para ese mes, o 0 si no hay ninguna
 */
export const getExchangeRateForDate = (rates, date) => {
    const globalRate = parseFloat(rates?.exchangeRate) || 0;
    const history = rates?.exchangeRateHistory;

    if (!history || Object.keys(history).length === 0) return globalRate;

    const d = date ? new Date(date) : new Date();
    const targetKey = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}`;

    // Buscar valor exacto del mes
    if (history[targetKey] !== undefined) return parseFloat(history[targetKey]);

    // Buscar el mes anterior más cercano disponible
    const sortedKeys = Object.keys(history).sort((a, b) => b.localeCompare(a)); // desc
    const nearest = sortedKeys.find(k => k <= targetKey);
    if (nearest) return parseFloat(history[nearest]);

    // Si todo falla, usar el global
    return globalRate;
};
...
export const calculateTicketFinancials = (ticket, rates, globalAssets, users = [], logisticsTasks = []) => {
    ...
    const billingDate = ticket.deliveryDetails?.customBillingDate || ticket.createdAt || new Date();
    ...
    if (isWarranty) {
        ...
        serviceRevenue = getRate(rates?.service_Other, rates?.otherService, 5); // Default base
    }
    ...
    if (logisticsTasks && logisticsTasks.length > 0) {
        subTasks = logisticsTasks.filter(task => String(task.ticket_id || task.ticketId) === String(ticket.id));
        if (subTasks.length === 0 && ticket.associatedCases) {
            const caseNumbers = ticket.associatedCases.map(c => String(c.caseNumber));
            subTasks = logisticsTasks.filter(task => caseNumbers.includes(String(task.case_number)) || caseNumbers.includes(String(task.ticketId)));
        }
    }
    if (subTasks.length === 0 && ticket.associatedCases) {
        subTasks = ticket.associatedCases;
    }

    if (subTasks.length > 0) {
        const taskFinancials = subTasks.map(task => calculateTaskFinancials(task, rates, globalAssets, users)).filter(Boolean);
        
        let totalSRevenue = taskFinancials.reduce((sum, f) => sum + f.serviceRevenue, 0);
        let totalLRevenue = taskFinancials.reduce((sum, f) => sum + f.logisticRevenue, 0);
        let totalLCost = taskFinancials.reduce((sum, f) => sum + f.logisticCost, 0);

        if (ticket.deliveryDetails?.hasSecondVisitSurcharge) {
            totalLRevenue *= 2;
            totalLCost *= 2;
        }

        // Apply custom override here too if present
        if (ticket.deliveryDetails?.customLogisticCostCurrency !== undefined && ticket.deliveryDetails?.customLogisticCostCurrency !== null) {
            if (ticket.deliveryDetails?.customLogisticCost !== undefined && ticket.deliveryDetails?.customLogisticCost !== null && ticket.deliveryDetails?.customLogisticCost !== '') {
                const val = parseFloat(ticket.deliveryDetails.customLogisticCost);
                if (ticket.deliveryDetails?.customLogisticCostCurrency === 'ARS') {
                    const rate = getExchangeRateForDate(rates, billingDate);
                    totalLCost = val / (rate > 0 ? rate : 1);
                } else {
                    totalLCost = val;
                }
            }
            ...
            // Legacy fallback
            if (ticket.deliveryDetails?.customLogisticCostARS !== undefined && ticket.deliveryDetails?.customLogisticCostARS !== null && ticket.deliveryDetails?.customLogisticCostARS !== '') {
                const rate = getExchangeRateForDate(rates, billingDate);
                totalLCost = parseFloat(ticket.deliveryDetails.customLogisticCostARS) / (rate > 0 ? rate : 1);
            } else if (ticket.deliveryDetails?.customLogisticCost !== undefined && ticket.deliveryDetails?.customLogisticCost !== null && ticket.deliveryDetails?.customLogisticCost !== '') {
                totalLCost = parseFloat(ticket.deliveryDetails.customLogisticCost);
            }
        }

        if (ticket.deliveryDetails?.customServiceRevenue !== undefined && ticket.deliveryDetails?.customServiceRevenue !== null && ticket.deliveryDetails?.customServiceRevenue !== '') {
            const val = parseFloat(ticket.deliveryDetails.customServiceRevenue);
            if (ticket.deliveryDetails?.customServiceRevenueCurrency === 'ARS') {
                const rate = getExchangeRateForDate(rates, billingDate);
                totalSRevenue = val / (rate > 0 ? rate : 1);
            } else {
                totalSRevenue = val;
            }
        }

        if (ticket.deliveryDetails?.customLogisticRevenue !== undefined && ticket.deliveryDetails?.customLogisticRevenue !== null && ticket.deliveryDetails?.customLogisticRevenue !== '') {
            const val = parseFloat(ticket.deliveryDetails.customLogisticRevenue);
            if (ticket.deliveryDetails?.customLogisticRevenueCurrency === 'ARS') {
                const rate = getExchangeRateForDate(rates, billingDate);
                totalLRevenue = val / (rate > 0 ? rate : 1);
            } else {
                totalLRevenue = val;
            }
        }
        ...
        let postalCostUSD = 0;
        if (ticket.deliveryDetails?.customPostalCost !== undefined && ticket.deliveryDetails?.customPostalCost !== null && ticket.deliveryDetails?.customPostalCost !== '') {
            const val = parseFloat(ticket.deliveryDetails.customPostalCost);
            if (!isNaN(val)) {
                if (ticket.deliveryDetails?.customPostalCostCurrency === 'ARS') {
                    const rate = getExchangeRateForDate(rates, billingDate);
                    postalCostUSD = val / (rate > 0 ? rate : 1);
                } else {
                    postalCostUSD = val;
                }
            }
        ...
        const totalC = totalLCost + postalCostUSD;
        ...
        return {
            ...
            logisticCost: totalLCost + postalCostUSD,
            baseLogisticCost: totalLCost,
            postalCost: postalCostUSD,
            operationalCost: 0, 
    ...
    if (ticket.deliveryDetails?.customLogisticCostCurrency !== undefined && ticket.deliveryDetails?.customLogisticCostCurrency !== null) {
        if (ticket.deliveryDetails?.customLogisticCost !== undefined && ticket.deliveryDetails?.customLogisticCost !== null && ticket.deliveryDetails?.customLogisticCost !== '') {
            ...
            if (ticket.deliveryDetails?.customLogisticCostCurrency === 'ARS') {
                const rate = getExchangeRateForDate(rates, billingDate);
                logisticCost = val / (rate > 0 ? rate : 1);
            } else {
                logisticCost = val;
            }
        }
        ...
        if (ticket.deliveryDetails?.customLogisticCostARS !== undefined && ticket.deliveryDetails?.customLogisticCostARS !== null && ticket.deliveryDetails?.customLogisticCostARS !== '') {
            const rate = getExchangeRateForDate(rates, billingDate);
            logisticCost = parseFloat(ticket.deliveryDetails.customLogisticCostARS) / (rate > 0 ? rate : 1);
        } else if (ticket.deliveryDetails?.customLogisticCost !== undefined && ticket.deliveryDetails?.customLogisticCost !== null && ticket.deliveryDetails?.customLogisticCost !== '') {
            logisticCost = parseFloat(ticket.deliveryDetails.customLogisticCost);
        }
    }
    ...
    if (ticket.deliveryDetails?.customServiceRevenue !== undefined && ticket.deliveryDetails?.customServiceRevenue !== null && ticket.deliveryDetails?.customServiceRevenue !== '') {
        ...
        if (ticket.deliveryDetails?.customServiceRevenueCurrency === 'ARS') {
            const rate = getExchangeRateForDate(rates, billingDate);
            serviceRevenue = val / (rate > 0 ? rate : 1);
        } else {
            serviceRevenue = val;
        }
    }
    ...
    if (ticket.deliveryDetails?.customLogisticRevenue !== undefined && ticket.deliveryDetails?.customLogisticRevenue !== null && ticket.deliveryDetails?.customLogisticRevenue !== '') {
        ...
        if (ticket.deliveryDetails?.customLogisticRevenueCurrency === 'ARS') {
            const rate = getExchangeRateForDate(rates, billingDate);
            logisticRevenue = val / (rate > 0 ? rate : 1);
        } else {
            logisticRevenue = val;
        }
    }
    ...
    const totalCost = logisticCost + operationalCost + postalCostUSD;
    const profit = totalRevenue - totalCost;

    return {
        serviceRevenue,
        logisticRevenue,
        logisticCost: logisticCost + postalCostUSD,
        baseLogisticCost: logisticCost,
        postalCost: postalCostUSD,
        operationalCost,
        totalRevenue,
        totalCost,
...
export const calculateTaskFinancials = (task, rates, globalAssets = [], users = []) => {
    ...
    if (isNoAction) {
        return {
            ...
            assetCases: [
                ...(task.assets || []).map(a => typeof a === 'object' && a !== null ? a.related_case : null),
                ...(task.yubikeys || []).map(y => typeof y === 'object' && y !== null ? y.related_case : null)
            ].filter(Boolean),
    ...
    if (isLaptop) {
        ...
        serviceRevenue = getRate(rates?.service_Other, rates?.otherService, 5); // Unknown device fallback
    }
