...
export function StoreProvider({ children }) {
    ...
    const addTicket = async (ticket) => {
        ...
        if (ticket.associatedCases && Array.isArray(ticket.associatedCases) && ticket.associatedCases.length > 0) {
            ...
            const tasksToInsert = ticket.associatedCases.map(ac => ({
                ...
                status: ac.logistics?.status || 'Para Coordinar',
                address: ac.logistics?.address || ticket.logistics?.address || '',
                method: ac.logistics?.method || ticket.logistics?.method || 'Por definir',
                delivery_person: ac.logistics?.deliveryPerson || ticket.logistics?.deliveryPerson || 'No asignado',
    ...
    const updateLogisticsTask = async (id, updatedData) => {
        ...
        const fields = [
            'status', 'method', 'delivery_person', 'assigned_to', 
            'date', 'time_slot', 'address', 'tracking_number', 
            'assets', 'accessories', 'yubikeys', 'delivery_info', 
            'subject', 'coordinated_by'
        ];

        fields.forEach(field => {
            if (updatedData[field] !== undefined) {
                dbUpdate[field] = updatedData[field];
            }
        });

        dbUpdate.updated_at = new Date().toISOString();

        const { error } = await supabase.from('logistics_tasks').update(dbUpdate).eq('id', id);
        if (error) {
