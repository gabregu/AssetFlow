...
export function StoreProvider({ children }) {
    ...
    const updateLogisticsTask = async (id, updatedData) => {
        ...
        // Sincronizamos el campo sfdc_case del activo apenas se vincula a una tarea
        if (updatedData.assets || task.assets) {
            ...
            if (ticketId && currentAssets.length > 0) {
                for (const item of currentAssets) {
                    ...
                    if (fullAsset && fullAsset.sfdcCase !== ticketId) {
                        console.log(`Sincronizando activo ${item.serial} con ticket ${ticketId}`);
                        await updateAsset(fullAsset.id, { sfdcCase: ticketId });
                    }
                }
            }
        }
