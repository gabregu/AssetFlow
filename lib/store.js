...
export function StoreProvider({ children }) {
            ...
            .on(
                ...
                (payload) => {
                    if (payload.eventType === 'INSERT') {
                        ...
                        setAssets(prev => prev.map(old => old.id === a.id ? formatted : old));
                    } else if (payload.eventType === 'DELETE') {
    ...
    const addAsset = async (asset) => {
        ...
        while (attempt < 5) {
            ...
            ['hardwareSpec', 'modelNumber', 'partNumber', 'purchaseOrder', 'sfdcCase', 'eolDate', 'imei2', 'boxNumber', 'dateLastUpdate', 'updatedBy', 'lastAssetCheck'].forEach(k => delete dbAsset[k]);
    ...
    const updateAsset = async (id, updatedData) => {
        ...
        const dbUpdate = {
            ...
            cod: updatedData.cod,
            last_asset_check: updatedData.lastAssetCheck || undefined
        };
        // Remove camelCase fields and frontend-only fields
        ['hardwareSpec', 'modelNumber', 'partNumber', 'purchaseOrder', 'sfdcCase', 'eolDate', 'imei2', 'boxNumber', 'dateLastUpdate', 'updatedBy', 'lastAssetCheck'].forEach(k => delete dbUpdate[k]);
        ...
        if (!error) {
            ...
            alert(`Error al actualizar el equipo: ${error.message}. ¿Ejecutaste la migración SQL 39?`);
        }
    };
    ...
    const clearInventoryLaptops = async (countryName) => {
        ...
        if (countryName && countryName !== 'Todos') {
            ...
            if (assetError) {
                ...
                setAssets(assets.filter(a => !(a.country === countryName && a.type === type)));
            }
        } else {
    ...
    const mapAssetToLocation = async (assetId, locationId) => {
        ...
        setAssets(prev => prev.map(a => a.id === assetId ? { ...a, locationId, dateMapped: now } : a));
        return { success: true };
    };
