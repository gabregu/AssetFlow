...
export function StoreProvider({ children }) {
    ...
    useEffect(() => {
        ...
        const fetchAppData = async () => {
            try {
                console.log("Fetching Critical App Data... (StoreProvider Init)");
                ...
                if (usersResult.status === 'fulfilled') {
                    ...
                    console.error("Critical User Fetch Failed:", usersResult.reason);
                }
                ...
                if (consResult.status === 'fulfilled') {
                    ...
                    console.log("DEBUG: Consumables Fetch Result:", { data, error }); // DEBUG LOG
                    if (!error && data) {
                        console.log("DEBUG: Setting Consumables State with length:", data.length); // DEBUG LOG
                        setConsumables(data);
                    }
                    else if (error) console.error("Error fetching consumables:", error);
                } else {
                    console.error("Critical Consumables Fetch Failed:", consResult.reason);
                }

                // UNBLOCK UI NOW
                setLoading(false);
                console.log("Critical Data Loaded (Partial or Full) - UI Unblocked");
                ...
                const heavyResults = await Promise.all([
                    ...
                    supabase.from('assets').select('*').range(0, 4999),
                ...
                const [
                    ...
                    { data: yubikeysData, error: yubikeysError },
                ...
                // Logistics Tasks (Relational Sub-Cases)
            ...
            try {
                ...
                if (session?.user) {
                    ...
                    if (profile) {
                        ...
                        if (profile && profile.email) { // Use 'profile' here as currentUser might not be updated yet
                            try {
    ...
    const addTicket = async (ticket) => {
        ...
        if (ticket.associatedCases && Array.isArray(ticket.associatedCases) && ticket.associatedCases.length > 0) {
            ...
            const tasksToInsert = ticket.associatedCases.map(ac => ({
                ...
                address: ac.logistics?.address || ticket.logistics?.address || '',
    ...
    const addAsset = async (asset) => {
        ...
        ['hardwareSpec', 'modelNumber', 'partNumber', 'purchaseOrder', 'sfdcCase', 'eolDate', 'imei2', 'country'].forEach(k => delete dbAsset[k]);

        const { error } = await supabase.from('assets').insert([dbAsset]);
        if (!error) {
            setAssets(prev => [{ ...asset, ...dbAsset, dateLastUpdate: now, updatedBy: dbAsset.updated_by }, ...prev]);
        }
    };
    ...
    const updateLogisticsTask = async (id, updatedData) => {
        console.log('=== [updateLogisticsTask] INICIO ===')
        console.log('+ ID de tarea:', id);
        console.log('+ Datos recibidos:', JSON.stringify(updatedData));
        ...
        const fieldMap = {
            ...
            'subject': 'subject'
            // 'coordinated_by' removido: columna no existe en la tabla logistics_tasks
        };
        ...
        console.log(`Updating Logistics Task ${id} with:`, dbUpdate);

        console.log('+ dbUpdate que se enviará a Supabase:', JSON.stringify(dbUpdate));
        
        if (Object.keys(dbUpdate).length === 0) {
            console.error('[updateLogisticsTask] ERROR: dbUpdate está vacío, no se enviará nada a la DB');
            return { error: new Error('dbUpdate vacío') };
        }
        ...
        if (error) {
            console.error('[updateLogisticsTask] ERROR de Supabase:', error);
            return { error };
        }
        
        console.log('[updateLogisticsTask] Respuesta de Supabase:', JSON.stringify(updateResult));
        
        if (!updateResult || updateResult.length === 0) {
            console.error('[updateLogisticsTask] ADVERTENCIA: Supabase no devolvió filas actualizadas. ¿El ID existe?', id);
        } else {
            console.log('[updateLogisticsTask] Éxito. Fecha guardada:', updateResult[0]?.date, '| Turno:', updateResult[0]?.time_slot);
        }
        
        setLogisticsTasks(prev => prev.map(t => t.id === id ? { ...t, ...updatedData } : t));
        
        // --- AUTOMATIZACIÓN: Cierre de Ticket Padre ---
        // Si la tarea se marcó como Entregado, verificamos el resto
        if (updatedData.status === 'Entregado') {
            // Find the task from the current state to get its ticket_id
            // Note: prev in setLogisticsTasks is the state *before* this update.
            // We need the *current* state to find the task with its ticket_id.
            // Or, more reliably, we can get the ticket_id from the original task object
            // if it was passed or stored, or fetch it.
            // For simplicity, let's assume the task object passed to updateLogisticsTask
            // already contains ticket_id, or we can find it from the current logisticsTasks state.
            const task = logisticsTasks.find(t => t.id === id);
            if (task && task.ticket_id) {
                // Filter all tasks related to this ticket_id, including the one just updated
                const allTasksForTicket = logisticsTasks.filter(t => 
                    String(t.ticket_id) === String(task.ticket_id)
                );
                
                // Check if ALL tasks for this ticket are now 'Entregado'
                const allDone = allTasksForTicket.every(t => 
                    (t.id === id && updatedData.status === 'Entregado') || // Current task with new status
                    (t.id !== id && t.status === 'Entregado') // Other tasks already 'Entregado'
                );
                
                if (allDone) {
                    console.log(`Todas las tareas del ticket ${task.ticket_id} están listas. Cerrando ticket...`);
                    await updateTicket(task.ticket_id, { status: 'Resuelto' });
                }
            }
        }
