...
export function StoreProvider({ children }) {
    ...
    useEffect(() => {
        ...
        const initAuth = async () => {
            ...
            // Failsafe: Force stop loading after 5 seconds to prevent infinite hang
            ...
            try {
                // Rely solely on onAuthStateChange's immediate INITIAL_SESSION event
                // to decide the loading state. This avoids race conditions on slow cookie loading.
                const authResponse = supabase.auth.onAuthStateChange(async (event, session) => {
                    ...
                    if (session?.user) {
                        ...
                        await refreshData();
                        if (!isUnmounted) setLoading(false); // Explicitly end loading after data is ready
                    } else {
                        if (!isUnmounted) {
                            setCurrentUser(null);
                            setLoading(false);
        ...
        const handleVisibilityChange = async () => {
            if (document.visibilityState === 'visible') {
                console.log("Tab visible again - verifying session and refreshing data");
                try {
                    // Force refresh token JWT if it expired in background (tab suspended by Chrome)
                    const { data: { session } } = await supabase.auth.getSession();
                    if (session) {
                        await refreshData();
                    } else {
                        console.warn("No active session found on tab focus.");
                    }
                } catch (err) {
