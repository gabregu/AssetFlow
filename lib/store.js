...
export function StoreProvider({ children }) {
    ...
    const updateAsset = async (id, updatedData) => {
        ...
        ['hardwareSpec', 'modelNumber', 'partNumber', 'purchaseOrder', 'sfdcCase', 'eolDate', 'imei2', 'boxNumber', 'dateLastUpdate', 'updatedBy'].forEach(k => delete dbUpdate[k]);
    ...
    const addConsumable = async (consumable) => {
        ...
        while (attempt < 5) {
            ...
            alert("Llamando insert de supabase...");
            
            // Timeout promise to catch hanging
            const timeoutPromise = new Promise((_, reject) => 
                setTimeout(() => reject(new Error("Timeout de Supabase tras 10 segundos!")), 10000)
            );

            let error = null;
            try {
                const result = await Promise.race([
                    supabase.from('consumables').insert([newConsumable]),
                    timeoutPromise
                ]);
                error = result.error;
            } catch (err) {
