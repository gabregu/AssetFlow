...
export function StoreProvider({ children }) {
    ...
    const refreshData = async () => {
        try {
            ...
            if (criticalResults[3] && criticalResults[3].status === 'fulfilled') {
                ...
                if (!error && data) {
                    setEntities(data);
                    // Si no hay filtro o está en 'Todos', seleccionamos el primero disponible
                    if (data.length > 0 && (countryFilter === 'Todos' || !countryFilter)) {
                        setCountryFilter(data[0].name);
                    }
                }
            }
                                            ...
                                            if (newState[key] && newState[key].length > 0) {
                                                ...
                                                if (!usersMap.has(uniqueId)) {
                                                    usersMap.set(uniqueId, {
                                                        email: key, // Now key is the user's email
                                                        ...presenceData
                                                    });
                                                }
                                            }
                                    ...
                                    .subscribe(async (status) => {
                                        if (status === 'SUBSCRIBED') {
                                            const userStatus = {
                                                ...
                                                email: profile.email, // Explicitly track email
    ...
    const addTicket = async (ticket) => {
        ...
        while (attempt < 5) {
            ...
            const dbData = {
                ...
                date: newTicket.date,
                client: newTicket.client || getClientName(countryFilter),
        ...
        if (ticket.associatedCases && Array.isArray(ticket.associatedCases) && ticket.associatedCases.length > 0) {
            ...
            tasksToInsert = ticket.associatedCases.map(ac => ({
                ticket_id: createdTicket.id,
                case_number: ac.caseNumber || ac.case_number || null,
            ...
            // Generar una tarea principal para tickets manuales sin número SFDC
            console.log(`Creando sub-caso manual por defecto para el ticket ${createdTicket.id}`);
            tasksToInsert = [{
                ticket_id: createdTicket.id,
                case_number: null,
                subject: 'Servicio Principal',
                status: ticket.logistics?.status || 'Pendiente',
                address: ticket.logistics?.address || null,
                method: ticket.logistics?.method || '',
                delivery_person: ticket.logistics?.deliveryPerson || 'Sin asignar',
                assigned_to: ticket.logistics?.assignedTo || null,
                date: ticket.logistics?.date || null,
                time_slot: ticket.logistics?.timeSlot || 'AM',
                assets: [],
                accessories: {}
            }];
        }
        ...
        if (tasksToInsert.length > 0) {
            console.log("addTicket: Inserting sub-tasks...", tasksToInsert);
    ...
    const updateAsset = async (id, updatedData) => {
        ...
        console.log(`Updating asset ${id}...`, dbUpdate);
    ...
    const updateLogisticsTask = async (id, updatedData) => {
        ...
        try {
            ...
            // Mantenemos el resultado pero NO retornamos aquí para permitir que la lógica de estado local y automatizaciones se ejecuten.
            // updateResultData = updateResult?.[0];
        } catch (err) {
    ...
    const updateWarehouseLocation = async (id, updatedData) => {
        const { data, error } = await supabase.from('warehouse_locations').update(updatedData).eq('id', id).select().single();
        if (error) {
    ...
    return (
        <StoreContext.Provider value={{
            ...
            addWarehouseLocation, updateWarehouseLocation, deleteWarehouseLocation, mapAssetToLocation
        }}>
            {children}
