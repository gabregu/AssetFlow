...
export function StoreProvider({ children }) {
    ...
    const [depositoConfig, setDepositoConfig] = useState({}); // Stores { "DEP-M": { rows: 3, cols: 15 } }
    ...
    const formatTicket = (t) => {
        ...
        return {
            ...
            salesforceCase: t.salesforce_case || null,
    ...
    const [isHeavyDataReady, setIsHeavyDataReady] = useState(false); // true cuando tickets, tasks, etc. están cargados
    ...
    const refreshData = async (force = false) => {
        ...
        try {
            ...
            const criticalPromises = [
                supabase.from('users').select('id, name, role, email, username, location_latitude, location_longitude, last_location_update, tracking_enabled, allowed_clients'),
                supabase.from('app_config').select('*').in('key', ['rates', 'deposito_config']),
            ...
            if (configResult.status === 'fulfilled') {
                ...
                if (!error && data) {
                    const ratesData = data.find(d => d.key === 'rates');
                    if (ratesData && ratesData.value) setRates({ ...initialRates, ...ratesData.value });
                    
                    const depConfigData = data.find(d => d.key === 'deposito_config');
                    if (depConfigData && depConfigData.value) setDepositoConfig(depConfigData.value);
                }
            }
            ...
            (async () => {
                try {
                    ...
                    setIsHeavyDataReady(true); // ✅ Marcar datos pesados como listos
    ...
    const addTicket = async (ticket) => {
        ...
        while (attempt < 5) {
            ...
            const dbData = {
                ...
                salesforce_case: newTicket.salesforceCase ? cleanString(String(newTicket.salesforceCase)) : null,
    ...
    const updateTicket = async (id, updatedData) => {
        try {
            ...
            if (updatedData.salesforceCase !== undefined) dbUpdate.salesforce_case = updatedData.salesforceCase ? cleanString(String(updatedData.salesforceCase)) : null;
            ...
            if (error) {
                ...
                // 2. PROPAGACIÓN EN CASCADA DE ESTADO "Bloqueado / A la Espera"
                if (updatedData.status === 'Bloqueado / A la Espera') {
                    const dependentTasks = logisticsTasks.filter(t => String(t.ticket_id) === String(id));
                    const tasksToUpdate = dependentTasks.filter(t => t.status !== 'Bloqueado / A la Espera');
                    ...
                    if (tasksToUpdate.length > 0) {
                        ...
                        supabase
                            ...
                            .update({ status: 'Bloqueado / A la Espera' })
                            .in('id', idsToUpdate)
                            .then(({ error: cascadeError }) => {
                                if (!cascadeError) {
                                    setLogisticsTasks(prev => prev.map(t => 
                                        idsToUpdate.includes(t.id) ? { ...t, status: 'Bloqueado / A la Espera' } : t
                                    ));
                                    console.log(`[Cascada] ${idsToUpdate.length} sub-tareas marcadas como Bloqueado / A la Espera.`);
                                } else {
                                    console.error('[Cascada] Error actualizando estado a Bloqueado:', cascadeError);
                                }
                            });
                    }
                }
    ...
    const updateAsset = async (id, updatedData) => {
        ...
        if (targetStatus === 'Asignado' || targetStatus === 'Por Recuperar') {
    ...
    const updateYubikey = async (id, updatedData) => {
        const dbUpdate = {};
        if (updatedData.type !== undefined || updatedData.name !== undefined || updatedData.model !== undefined) {
            dbUpdate.type = updatedData.type || updatedData.name || updatedData.model;
        }
        if (updatedData.serial !== undefined) dbUpdate.serial = updatedData.serial ? String(updatedData.serial).trim() : null;
        if (updatedData.status !== undefined) dbUpdate.status = updatedData.status;
        if (updatedData.country !== undefined || updatedData.region !== undefined) {
            dbUpdate.country = updatedData.country || updatedData.region;
        }
        if (updatedData.assignee !== undefined) dbUpdate.assignee = updatedData.assignee;

        const { error } = await supabase.from('yubikeys').update(dbUpdate).eq('id', id);
        if (!error) {
            setYubikeys(prev => prev.map(y => y.id === id ? { ...y, ...updatedData, ...dbUpdate } : y));
        } else {
            console.error("Error updating yubikey:", error);
            throw error;
        }
    };

    const addYubikey = async (yubikey) => {
        const yubiType = (yubikey.type || yubikey.name || yubikey.model || 'YubiKey 5 NFC').trim();
        const yubiSerial = yubikey.serial ? String(yubikey.serial).trim() : '';
        const yubiStatus = yubikey.status || 'Nuevo';
        const yubiCountry = yubikey.country || yubikey.region || 'Argentina';
        const yubiAssignee = yubikey.assignee || null;
        ...
        const dbRecord = {
            ...
            status: yubiStatus,
            country: yubiCountry,
            assignee: yubiAssignee,
            add_by_user: currentUser?.name || 'Admin',
            created_at: new Date().toISOString()
        };

        const { data, error } = await supabase
            .from('yubikeys')
            .upsert([dbRecord], { onConflict: 'serial' })
            .select();

        if (error) {
            console.error("Error adding/upserting yubikey:", error);
            throw error;
        }
        ...
        if (data && data.length > 0) {
            setYubikeys(prev => [
                data[0],
                ...prev.filter(y => y.id !== data[0].id && String(y.serial || '').trim() !== String(data[0].serial || '').trim())
            ]);
            return data[0];
        }
    };
    ...
    const updateRates = async (newRates, silent = false) => {
        try {
            ...
            alert(`Excepción inesperada: ${e.message}`);
        }
    };

    const updateDepositoConfig = async (newConfig) => {
        try {
            const { error } = await supabase.from('app_config').upsert({ key: 'deposito_config', value: newConfig });
            if (error) {
                console.error('Error saving deposito_config:', error);
                alert("Error al guardar dimensiones de repisa.");
            } else {
                setDepositoConfig(newConfig);
            }
        } catch (e) {
            console.error('Exception updating deposito_config:', e);
        }
    };
    ...
    const updateLogisticsTask = async (id, updatedData) => {
        try {
                ...
                if (ticketId && newAssets.length > 0) {
                    for (const item of newAssets) {
                        ...
                        if (fullAsset) {
                            ...
                            const targetStatus = isRecovery ? 'Por Recuperar' : 'Asignado';
                            const newAssignee = isRecovery 
                                ? (fullAsset.assignee && fullAsset.assignee !== 'Almacén' ? fullAsset.assignee : (parentTicket?.requester || 'Usuario Final'))
                                : (parentTicket?.requester || 'Usuario Final');
                            ...
                            if (fullAsset.sfdcCase !== targetSfdcCase || fullAsset.status !== targetStatus || fullAsset.assignee !== newAssignee) {
                                console.log(`Sincronizando activo ${item.serial} con ticket ${ticketId} / caso ${targetSfdcCase} -> Estado: ${targetStatus}`);
                                await updateAsset(fullAsset.id, { 
                                    sfdcCase: targetSfdcCase,
                                    status: targetStatus,
                                    assignee: newAssignee,
                                    notes: (fullAsset.notes ? fullAsset.notes + '\n' : '') + 
                                           `[${new Date().toLocaleDateString()}] ASOCIADO/ACTUALIZADO en Ticket #${ticketId} (Caso: ${targetSfdcCase}) -> Estado: ${targetStatus} (Asignado a: ${newAssignee})`
                                });
                            }
                        }
                    }
            ...
            if (updatedData.status === 'Entregado' || updatedData.status === 'Recuperado') {
                    ...
                    for (const item of finalAssets) {
                        ...
                        if (fullAsset) {
                            ...
                            await updateAsset(fullAsset.id, {
                                ...
                                sfdcCase: targetSfdcCase,
                                notes: (fullAsset.notes ? fullAsset.notes + '\n' : '') + 
                                       `[${new Date().toLocaleDateString()}] ${isRecovery ? 'RECUPERADO (Ingresado a Verificación HW)' : 'ENTREGADO'} vía Ticket #${parentTicket?.id || task.ticket_id} (${taskCaseNumber || 'Sub-caso'})`
                            });
                        }
                    }
                }
    ...
    const updateEntity = async (id, updatedData) => {
        // Obtener el nombre viejo antes de actualizar
        ...
        // Actualización en cascada: si se renombró la entidad, propagar a todas las tablas relacionadas
        if (oldName && newName && oldName !== newName) {
            console.log(`Renombrando entidad "${oldName}" → "${newName}" en cascada...`);

            // 1. Actualizar tickets.client
            await supabase.from('tickets').update({ client: newName }).eq('client', oldName);

            // 2. Actualizar assets.country
            await supabase.from('assets').update({ country: newName }).eq('country', oldName);

            // 3. Actualizar yubikeys.country
            await supabase.from('yubikeys').update({ country: newName }).eq('country', oldName);

            // 3. Actualizar users.allowed_clients (array que contiene el nombre viejo)
            const usersWithOldName = users.filter(u =>
                Array.isArray(u.allowed_clients) && u.allowed_clients.includes(oldName)
            );
            for (const u of usersWithOldName) {
                const updatedClients = u.allowed_clients.map(c => c === oldName ? newName : c);
                await supabase.from('users').update({ allowed_clients: updatedClients }).eq('id', u.id);
            }

            // Refrescar datos locales para reflejar los cambios
            await refreshData();
        }
    ...
    return (
        <StoreContext.Provider value={{
            ...
            clearInventory, clearInventoryLaptops, clearInventorySmartphones, rates, updateRates, depositoConfig, updateDepositoConfig, addExpense, deleteExpense,
            ...
            onlineUsers, refreshData, setLoading, isHeavyDataReady,
