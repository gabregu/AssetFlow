...
export function StoreProvider({ children }) {
    ...
    const updateLogisticsTask = async (id, updatedData) => {
        ...
        // Sincronizamos el estado, asignado y caso SFDC del activo apenas se vincula a una tarea
        if (updatedData.assets || task.assets) {
            const ticketId = updatedData.ticket_id || task.ticket_id;
            const parentTicket = tickets.find(t => t.id === ticketId);
            const currentAssets = updatedData.assets || task.assets || [];
            
            if (ticketId && currentAssets.length > 0) {
                for (const item of currentAssets) {
                    const fullAsset = assets.find(a => a.serial.toLowerCase() === item.serial?.toLowerCase());
                    if (fullAsset && (fullAsset.sfdcCase !== ticketId || fullAsset.status !== 'Asignado')) {
                        console.log(`Sincronizando activo ${item.serial} con ticket ${ticketId} y responsable ${parentTicket?.requester}`);
                        ...
                        await updateAsset(fullAsset.id, { 
                            ...
                            notes: (fullAsset.notes ? fullAsset.notes + '\n' : '') + 
                                   `[${new Date().toLocaleDateString()}] ASOCIADO a Ticket #${ticketId} (Asignado a: ${newAssignee})`
                        });
                    }
                }
            }
