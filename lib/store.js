            ...
            try {
                ...
                const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, session) => {
                    ...
                    if (session?.user) {
                        ...
                        // Using filter with ilike for safety across versions
                        let profile = null;
                        try {
                            const { data, error } = await supabase
                                .from('users')
                                .select('*')
                                .eq('email', session.user.email?.toLowerCase()) // Normalize to lowercase for matching
                                .maybeSingle();
                        ...
                        // Debugging profile fetch
                        console.log('Profile Fetch Result:', profile);

                        if (profile) {
                            setCurrentUser({ ...profile, id: profile.id, uuid: session.user.id });
                        } else {
                            // Fallback if no profile is found yet (sync delay?)
                            console.warn('Profile not found in public.users, falling back to default user role.');
                            setCurrentUser({
                                id: session.user.id,
                                email: session.user.email,
                                role: 'user', // Default role cleanup
                ...
                const { data: usersData, error: usersError } = await supabase
                    .from('users')
                    .select('id, name, role, email, username, location_latitude, location_longitude, last_location_update, tracking_enabled');
                ...
                if (usersError) {
                    ...
                    console.warn("No users found (RLS blocking or empty table?)");
                }
