...
export function StoreProvider({ children }) {
    ...
    const refreshData = async () => {
        ...
        if (nowTime - lastRefreshTimeRef.current < 5000) {
        ...
        const verifyAndRefreshSession = async () => {
            try {
                ...
                if (session) {
                    ...
                    // If expired or expiring in less than 10 minutes (600 seconds)
                    if (expiresAt - now < 600) {
                        console.log("JWT is close to expiration or already expired in background. Forcing token refresh...");
                        const refreshRes = await supabase.auth.refreshSession();
                        const newSession = refreshRes?.data?.session;
                        const error = refreshRes?.error;
                ...
                console.error("Error verifyAndRefreshSession:", err);
            }
            return null;
        };

        // Tiempo mínimo entre refrescos disparados por foco/visibilidad (5 minutos)
        // Evita recargar datos cuando el usuario descarga un PDF, abre una pestaña nueva, etc.
        const FOCUS_REFRESH_COOLDOWN_MS = 15 * 60 * 1000;
        let lastFocusRefreshTime = Date.now();

        const handleVisibilityChange = async () => {
            if (document.visibilityState === 'visible') {
                const now = Date.now();
                if (now - lastFocusRefreshTime < FOCUS_REFRESH_COOLDOWN_MS) {
                    console.log("Tab visible - skipping refresh (cooldown activo)");
                    return;
                }
                ...
                try {
                    ...
                    if (session) {
                        ...
                        console.warn("No active session found on visibility change.");
                    }
                } catch (err) {
                    console.error("Error verifying session on visibility change:", err);
                }
            }
        };
    ...
    const addAsset = async (asset) => {
        ...
        while (attempt < 5) {
            ...
            ['hardwareSpec', 'modelNumber', 'partNumber', 'purchaseOrder', 'sfdcCase', 'eolDate', 'imei2', 'boxNumber', 'dateLastUpdate', 'updatedBy', 'lastAssetCheck', 'locationId', 'dateMapped', 'addByUser'].forEach(k => delete dbAsset[k]);
    ...
    const updateTicket = async (id, updatedData) => {
        try {
            ...
            if (updatedData.status !== undefined) {
                ...
                const isTransitioningToResolved = resolvedStatuses.includes(updatedData.status) && 
            ...
            const cleanString = (str) => {
                ...
                // Strip invisible characters (Excel paste artifacts, zero-width spaces, LRM, RLM, control chars)
            ...
            const withTimeout = (promise, ms) => {
                ...
                const timeoutPromise = new Promise((_, reject) => {
                    timeoutId = setTimeout(() => {
                        reject(new Error(`Timeout: La base de datos no respondió tras ${ms/1000}s. Reintenta.`));
                    }, ms);
                });
                // Supabase returns a PostgrestBuilder which is thenable but lacks .finally()
                // Wrapping it in Promise.resolve() converts it to a native Promise
                return Promise.race([
            ...
            let { error } = await withTimeout(supabase.from('tickets').update(dbUpdate).eq('id', id), 15000);

            if (errSpan) errSpan.textContent = "2.1.2 BD respondió...";
            ...
            if (error && (error.status === 401 || error.code === 'PGRST301' || error.message?.includes('JWT') || error.message?.includes('claim'))) {
                console.warn("Update ticket failed due to JWT expiration. Attempting automatic recovery...");
                if (errSpan) errSpan.textContent = "2.1.3 Recuperando sesión...";
                const refreshRes = await withTimeout(supabase.auth.refreshSession(), 10000);
                const session = refreshRes?.data?.session;
                if (session) {
                    console.log("Session refreshed successfully. Retrying ticket update...");
                    if (errSpan) errSpan.textContent = "2.1.4 Reintentando...";
                    const retryResult = await withTimeout(supabase.from('tickets').update(dbUpdate).eq('id', id), 15000);
    ...
    const deleteTickets = async (ids) => {
        try {
            ...
            if (error) {
                ...
                setAssets(prev => prev.map(a => ids.includes(a.sfdcCase) ? { ...a, sfdcCase: null } : a));
    ...
    const updateAsset = async (id, updatedData) => {
        ...
        const dbUpdate = {
            name: cleanString(updatedData.name),
            type: updatedData.type,
            serial: cleanString(updatedData.serial),
            status: updatedData.status,
            assignee: cleanString(updatedData.assignee),
            date: updatedData.date,
            notes: cleanString(updatedData.notes),
            country: updatedData.country,
            vendor: cleanString(updatedData.vendor),
            oem: cleanString(updatedData.oem),
            imei: cleanString(updatedData.imei),
            hardware_spec: cleanString(updatedData.hardwareSpec),
            model_number: cleanString(updatedData.modelNumber),
            part_number: cleanString(updatedData.partNumber),
            purchase_order: cleanString(updatedData.purchaseOrder),
            sfdc_case: cleanString(updatedData.sfdcCase),
            eol_date: updatedData.eolDate,
            imei_2: cleanString(updatedData.imei2),
            box_number: cleanString(updatedData.boxNumber),
            cod: cleanString(updatedData.cod),
    ...
    const updateLogisticsTask = async (id, updatedData) => {
        try {
            ...
            if (task && task.ticket_id) {
                ...
                // Guardar historial en el ticket padre silenciosamente
                if (newHistoryNotes.length > 0) {
                    ...
                    if (parentTicket) {
                        ...
                        await updateTicket(parentTicket.id, { internalNotes: finalNotes });
                    }
                }
            }
            // -----------------------------------
            ...
            Object.keys(fieldMap).forEach(key => {
                if (updatedData[key] !== undefined) {
                    ...
                    // Limpiar strings de espacios o saltos de línea copiados (Excel)
            ...
            // Auto-retry once on authorization / JWT errors
            if (error && (error.status === 401 || error.code === 'PGRST301' || error.message?.includes('JWT') || error.message?.includes('claim'))) {
                console.warn("Update logistics task failed due to JWT expiration. Attempting automatic recovery...");
                const refreshRes = await supabase.auth.refreshSession();
                const session = refreshRes?.data?.session;
                if (session) {
                    console.log("Session refreshed successfully. Retrying logistics task update...");
