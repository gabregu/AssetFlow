    ...
    useEffect(() => {
        ...
        const handleVisibilityChange = async () => {
            if (document.visibilityState === 'visible' || document.hasFocus()) {
                console.log("Tab focused or visible - verifying session validity");
                try {
                    const session = await verifyAndRefreshSession();
                    if (session) {
                        await refreshData();
                    } else {
                        console.warn("No active session found on focus/visibility change.");
                    }
                } catch (err) {
                    console.error("Error verifying session on visibility/focus change:", err);
                }
            }
        };
