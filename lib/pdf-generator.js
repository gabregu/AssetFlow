...
export const generateTicketPDF = (ticket, assets, deliveryData = null, action = 'download') => {
    ...
    // Check if there are specific models selected to avoid duplicating the generic entries
    const customKeys = Object.keys(ticket.accessories || {}).filter(k => !legacyKeys.includes(k) && ticket.accessories[k] === true);
    ...
    if (ticket.accessories?.backpack && !hasSpecificBackpack) taskRows.push([commonTypeLabel, 'Accesorio', 'Mochila Técnica', '-']);
    if (ticket.accessories?.screenFilter && !hasSpecificFilter) taskRows.push([commonTypeLabel, 'Accesorio', `Filtro de Pantalla ${ticket.accessories.filterSize || ''}`.trim(), '-']);
    ...
    if (action === 'view') {
        ...
        try {
            const fileName = `Remito_${ticket.id}.pdf`;
            const blob = doc.output('blob');
            const file = new File([blob], fileName, { type: 'application/pdf' });
            if (navigator.share && navigator.canShare && navigator.canShare({ files: [file] })) {
                navigator.share({
                    files: [file],
                    title: `Remito ${ticket.id}`,
                    text: `Adjunto el remito PDF correspondiente al ticket ${ticket.id}.`
                }).catch(err => {
