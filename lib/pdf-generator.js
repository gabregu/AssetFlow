...
export const generateTicketPDF = async (ticket, assets, deliveryData = null, action = 'download') => {
    const doc = new jsPDF();
    ...
    consolidatedAssets.forEach(item => {
        ...
        // Filter out empty hardware entries
        if ((serial === '-' || serial === '') && (desc === '-' || desc === '')) {
             return;
        }
    ...
    if (signatureDataUrl) {
        ...
        // El QR sólo debe generarse para envíos por Correo / encomienda.
        // Las entregas puerta a puerta (chofer / cadete local) no llevan QR y mantienen el recuadro limpio para firma física.
        ...
        const trackingNum = String(
            ticket.logistics?.trackingNumber || 
            ticket.logistics?.tracking_number || 
        ...
        const isPostalDelivery = 
            ...
            driverStr.includes('dhl') || 
            driverStr.includes('fedex') || 
            (trackingNum.length > 3 && trackingNum !== '-');

        if (isPostalDelivery) {
            try {
                const ticketId = ticket.id || deliveryData?.ticketId;
                if (ticketId) {
                    const caseNum = ticket.caseNumber || sfdcId || '';
                    const recipient = ticket.logistics?.contactName || ticket.requester || recipientName || '';
                    const origin = typeof window !== 'undefined' ? window.location.origin : 'https://assetflow-yawi.vercel.app';
                    
                    let confirmUrl = '';
                    try {
                        const res = await fetch(`${origin}/api/confirm-delivery?action=get-token&ticketId=${encodeURIComponent(ticketId)}&caseNumber=${encodeURIComponent(caseNum)}&recipient=${encodeURIComponent(recipient)}`);
                        if (res.ok) {
                            const tokenData = await res.json();
                            if (tokenData?.url) confirmUrl = tokenData.url;
                        }
                    } catch {
                        // Si falla el fetch, usamos generación local
                    }

                    if (!confirmUrl) {
                        const deliveryToken = generateDeliveryToken({ ticketId, caseNumber: caseNum, recipient });
                        confirmUrl = `${origin}/entrega/${deliveryToken}`;
                    }
                ...
                const qrBoxWidth = pageWidth - 20 - qrBoxX; // ~72mm
                const qrBoxHeight = 36;
                const qrSize = 32;
                ...
                const textX = qrBoxX + qrSize + 4;
                doc.setFont('helvetica', 'bold');
                doc.setFontSize(7.5);
                doc.setTextColor(30, 58, 138);
                doc.text('FIRMA DIGITAL', textX, qrBoxY + 8);

                doc.setFont('helvetica', 'normal');
                doc.setFontSize(6.5);
                doc.setTextColor(71, 85, 105);
                doc.text('Escanea con tu celular', textX, qrBoxY + 14);
                doc.text('para firmar y confirmar', textX, qrBoxY + 18);
                doc.text('la recepcion en linea.', textX, qrBoxY + 22);

                doc.setFont('helvetica', 'italic');
                doc.text('Sin login requerido', textX, qrBoxY + 29);
            }
        } catch (qrErr) {
            console.error('Error generando QR para remito:', qrErr);
        }
    }
}
