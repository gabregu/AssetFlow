...
export const generateTicketPDF = (ticket, assets, deliveryData = null, action = 'download') => {
    ...
    if (deliveryData?.notes || ticket.deliveryDetails?.notes) {
        doc.setFont('helvetica', 'normal');
        doc.text(cleanText(deliveryData?.notes || ticket.deliveryDetails?.notes || ''), 20, currentY + 7, { maxWidth: pageWidth - 40 });
    }
    ...
    autoTable(doc, {
        ...
        tableWidth: 90,
        body: [
            ['Persona que entregó o recibió:', cleanText(deliveryData?.receivedBy || ticket.deliveryDetails?.receivedBy || '')],
