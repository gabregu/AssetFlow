...
export const generateTicketPDF = (ticket, assets, deliveryData = null, action = 'download') => {
    ...
    autoTable(doc, {
        ...
        body: [
            ...
            ['Dirección:', { content: cleanText(ticket.logistics?.address), colSpan: 3 }]
        ],
        theme: 'grid',
    ...
    doc.setFontSize(10);
    doc.text(cleanText(ticket.subject), 20, currentY + 7, { maxWidth: pageWidth - 40 });
    ...
    autoTable(doc, {
        ...
        tableWidth: 90,
        body: [
            ['Persona que entregó o recibió:', cleanText(deliveryData?.receivedBy || ticket.logistics?.deliveryInfo?.receivedBy || ticket.deliveryDetails?.receivedBy || '')],
            ['DNI:', deliveryData?.dni || ticket.logistics?.deliveryInfo?.dni || ticket.deliveryDetails?.dni || ''],
            ['Dia:', (() => {
                const rawDate = deliveryData?.deliveredAt || ticket.logistics?.deliveryInfo?.deliveredAt || ticket.deliveryDetails?.deliveredAt || ticket.deliveryCompletedDate;
                if (!rawDate) return '-';
                try {
            ...
            ['Hora:', deliveryData?.actualTime || ticket.logistics?.deliveryInfo?.actualTime || ticket.deliveryDetails?.actualTime || '']
        ],
        theme: 'grid',
        styles: { fontSize: 9, cellPadding: 3 },
        columnStyles: {
...
export const generateLabelPDF = async (ticket, assets) => {
    ...
    addLine('Dirección', cleanText(ticket.logistics?.address));
