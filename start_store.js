...
export function StoreProvider({ children }) {
    ...
    const refreshData = async () => {
        try {
            ...
            if (tasksData) setLogisticsTasks(tasksData.map(t => ({ ...t, caseNumber: t.case_number })));
    ...
    const clearSfdcCases = async (country) => {
        ...
        // Si se provee un pa??s v??lido que no sea 'Todos', filtramos la eliminaci??n por pa??s
        if (country && country !== 'Todos') {
            query = query.ilike('country', `%${country}%`); // Usar ilike para b??squeda flexible (ej. "chile" == "Chile")
        }

        const { error } = await query;
        if (!error) {
            if (country && country !== 'Todos') {
                // Borrar solo los de ese pa??s del estado local (ignorando may??sculas/min??sculas)
    ...
    const addTicket = async (ticket) => {
        ...
        // --- AUTOMATIZACI??N: Crear Sub-Casos (logistics_tasks) autom??ticamente ---
        if (ticket.associatedCases && Array.isArray(ticket.associatedCases) && ticket.associatedCases.length > 0) {
            ...
            if (taskError) {
                console.error("Error creando sub-casos autom??ticos:", taskError);
    ...
    const addAssets = async (newAssets) => {
        ...
        while (attempt < 5) {
            ...
            const formattedAssets = newAssets.map((asset, index) => {
                ...
                const dbAsset = {
                    ...
                    assignee: asset.assignee || 'Almac??n',
                    updated_by: currentUser?.name || 'Importaci??n',
                    date_last_update: now,
                    hardware_spec: asset.hardwareSpec,
                    model_number: asset.modelNumber,
                    part_number: asset.partNumber,
                    ...
                    name: asset.name,
                    serial: asset.serial,
                    add_by_user: currentUser?.name || 'Importaci??n'
                };
                ['hardwareSpec', 'modelNumber', 'partNumber', 'purchaseOrder', 'sfdcCase', 'eolDate', 'imei2'].forEach(k => delete dbAsset[k]);
                return dbAsset;
            });
        ...
        if (!success) {
            throw new Error("No se pudo completar la importaci??n debido a colisiones constantes de IDs.");
        }
    };
    ...
    const deleteTicket = async (id) => {
        ...
        if (!error) {
            ...
            // Actualizar estado local de activos tambi??n
    ...
    const deleteTickets = async (ids) => {
        try {
            ...
            if (assetError) {
                ...
                // No detenemos el proceso, pero lo logueamos. Si falla esto, el delete de tickets podr??a fallar si hay FK restrict.
            }
    ...
    const addConsumable = async (consumable) => {
        ...
        if (!createdConsumable) {
            throw new Error("No se pudo generar un ID ??nico para el consumible despu??s de varios intentos/consultas.");
        }
    };
