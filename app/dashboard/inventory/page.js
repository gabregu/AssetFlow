...
export default function InventoryPage() {
    ...
    const handleCreateOrUpdate = async (e) => {
        ...
        if (editingAsset && isNoDevuelto) {
            ...
            try {
                ...
                const targetCaseNum = editingAsset.sfdcCase || newAsset.sfdcCase;
                const targetSerial = (editingAsset.serial || newAsset.serial || '').trim().toLowerCase();
                ...
                if (matchingTicket && updateTicket) {
                    const noteText = `[${nowFormatted}] ⚠️ DISPOSITIVO NO DEVUELTO: El equipo ${editingAsset.name || editingAsset.model || 'Dispositivo'} (S/N: ${editingAsset.serial}) fue marcado como "No Devuelto" y borrado del inventario. El dispositivo NO fue devuelto por parte del usuario: ${targetUser}. (Registrado por: ${updatedBy})`;
                ...
                console.error('Error al procesar activo no devuelto:', err);
    ...
    const handleAddYubikey = async (e) => {
        e.preventDefault();
        try {
            const yubiType = newYubikey.type || newYubikey.name || 'YubiKey 5 NFC';
            await addYubikey({ 
                type: yubiType,
                serial: newYubikey.serial,
                status: newYubikey.status,
        ...
        <div style={{ paddingBottom: '4rem', animation: 'fadeIn 0.5s ease-out' }}>
            ...
            <Modal
                ...
                onClose={() => {
                    ...
                    setIsYubikeyModalOpen(false);
                    setSelectedYubikey(null);
                    setNewYubikey({ type: 'YubiKey 5 NFC', serial: '', status: 'Nuevo', country: 'Argentina' }); // Reset form
                }}
            ...
            >
                <form onSubmit={(e) => {
                    ...
                    <div className="form-group">
                        <label className="form-label">Modelo / Tipo de Security Key</label>
                        <input
                            type="text"
                            required
                            className="form-input"
                            value={selectedYubikey ? (selectedYubikey.type || selectedYubikey.name || '') : (newYubikey.type || newYubikey.name || '')}
                            onChange={(e) => selectedYubikey ? setSelectedYubikey({ ...selectedYubikey, type: e.target.value }) : setNewYubikey({ ...newYubikey, type: e.target.value })}
            ...
            <QRScannerModal 
                ...
                onScanSuccess={(result) => {
                    ...
                    // Poner el serial en el buscador y resetear todos los filtros
                    // (mismo comportamiento que escribir en el input de búsqueda)
