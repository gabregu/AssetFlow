...
export default function HistoryPanel({ ticket, editedData, setEditedData, updateTicket, currentUser }) {
    ...
    return (
        ...
        >
            {/* Input area - ALWAYS VISIBLE for quick updates */}
            <div style={{ marginBottom: isOpen ? '2rem' : '0', background: 'var(--surface)', padding: '0.75rem', borderRadius: 'var(--radius-md)', border: '1px solid var(--border)' }}>
                <textarea
                    placeholder="Añadir una nota al historial..."
                    className="form-textarea"
                    style={{ minHeight: '60px', resize: 'none', border: 'none', background: 'transparent', width: '100%', fontSize: '0.9rem' }}
                    value={newNote}
                    onChange={e => setNewNote(e.target.value)}
                />
                <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '0.5rem', borderTop: '1px solid var(--border)', paddingTop: '0.5rem' }}>
                    <Button size="sm" onClick={handleAddNote} disabled={!newNote.trim()}>Registrar</Button>
                </div>
            </div>

            {isOpen && (
                <div style={{ borderLeft: '2px solid var(--border)', paddingLeft: '1.5rem', marginLeft: '0.5rem', display: 'flex', flexDirection: 'column', gap: '1.5rem', marginTop: '1.5rem', animation: 'fadeIn 0.3s ease-out' }}>
